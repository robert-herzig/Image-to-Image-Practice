...
def color_error_image(errors, scale=1, mask=None, BGR=True):
    ...
    colored_errors_flat = color_map_errors[i0, :] * (1 - f1).reshape(-1, 1) + color_map_errors[i0 + 1, :] * f1.reshape(
        -1, 1)
...
def color_trust_image(trust, mask=None, BGR=True):
    """
    Color an input trust map.

    Arguments:
        trust -- HxW numpy array of trust
        [mask=None] -- zero-pixels are masked white in the result
        [BGR=True] -- toggle between BGR and RGB

    Returns:
        colored_trust -- HxWx3 numpy array visualizing the trust
    """

    return color_error_image(np.power(2.0, 4 - trust * 8), scale=1, mask=mask, BGR=BGR)
...
def color_depth_map(depths, scale=None, mask=None):
    """
    ...
    Arguments:
        ...
        [mask=None] -- optionally masking a set of pixels as white
    ...
    if mask is not None:
        ...
        colors = colors * mask + (1 - mask) * 255.0
    return colors.reshape(depths.shape[0], depths.shape[1], 3)
